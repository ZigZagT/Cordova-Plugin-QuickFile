...
var QuickFile = {
	...
	readDirectory: function(dirFullPath,success,fail){
	...
	mkDirectory: function(dirFullPath,success,fail){
		var existPath = QuickFile.extractDirectory(dirFullPath);
		var createPath = "";
		function shiftPath(e, fail) {
			if (e.code == FileError.NOT_FOUND_ERR) {
				existPath = QuickFile.extractDirectory(existPath);
				createPath = dirFullPath.replace(existPath, "");
				createPath = QuickFile.extractDirectory(createPath);
		...
		function finish(entry) {
			...
			if (createPath.indexOf('/') === -1) {
				...
				createPath = createPath.substring(0, createPath.indexOf('/'));
				entry.getDirectory(createPath, {create: true, exclusive: false}, function(){
					QuickFile.mkDirectory(dirFullPath, success, fail);
				}, fail);
			}
